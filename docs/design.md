...
- Phase 3 is done (closed-day rule, terciles, subset; see the decision log). Open from it:
  - **Stock-out runs (Phase 4/5):** decide how to treat zero runs of 14+ days after the first sale, which are probably stock-outs or delistings. Options: leave as is and state it as limitation 2 quantified (default, consistent with the design's "forecast sales as a proxy for demand"); add a "long zero run" feature (days since last sale is already planned); or drop fit and calibration windows that sit inside such runs. Whichever is chosen must apply identically to both policies, and the simulator's replay of zero demand inside such runs will flatter every policy on inventory.
  - **Priced-but-unsold history (Phase 4):** `FOODS_2_101_CA_2` (first sale 1,578 days after its first price). Keep, or start its series at the first sale. One series, so low stakes.
