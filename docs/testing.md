...
| Policy quantities (9) | `test_policy.py` (16 tests, hand-computed): classic ROP (yhat 10, sigma 0.5, scale 4, alpha 0.9 -> 12.563 -> 13, SS 3), quantile ROP (12.01 -> 13, exact 12.0 -> 12, noise guard), integer ceiling, monotonicity in alpha and inputs, label boundaries (Q50 5, Q90 10, Q99 16: IP 4 HIGH, 5 and 9 MEDIUM, 10 and 16 LOW, 17 overstock), degenerate quantiles, unsorted quantiles rejected, constant-CV check on synthetic segments of known structure, sigma by hand | done |
| Demo API and data (11) | `server/test/app.test.js` (14 Node tests): `/health` 200 with the version and no dependency call; `/health/deep` 200, and 503 with a short reason for database down, data-version mismatch or missing; no health or error response leaks secrets, hashes, paths or SQL; rate limiting (general and `/health/deep`); the what-if `max(0, ceil(q) - position)` by hand, ceil noise, band boundaries and both sides of a ceiled tie, the two label qualifiers, input validation, scenario ids. `tests/test_demo_data.py` (snapshot built from the result files, B3a first, logged numbers, no series ids or dates, version in step with Express). `tests/test_api.py` (8 tests): FastAPI health, API key, hash mismatch, lock verification, exported model reproduces the research model, scaling. Integration checked by hand against Postgres (compose): load, what-if against a stored quantile, database stopped and restarted, stale data version, the publication gate | done locally; **not deployed** |
| Simulator and policies (10) | `test_simulator.py` (34 tests): hand-computed path, orders, receipts, lost sales, fill rate and inventory; timing (arrival before day t+L+1, protection interval t+1..t+R+L); conservation invariants over seeded random demand; integer inputs enforced; limit cases (S = 0, huge S, zero demand, closed days, naive c = 1); leakage (decisions at or before review t do not depend on later demand); an independent event-style reference implementation for L = 0, 3, 7; matched-service statistic in both anchors by hand; bootstrap item clusters; multi-unit levels, calibration per unit, sub-period rule, touch-log guard | done (also covers the lead-array, crossing-order and zero-demand-mask engine features and the sensitivity helpers; the optional backorder variant is not built) |
