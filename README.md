...
**Inventory replay on the test window** ([phase 10 primary run](docs/results/phase10_simulator_primary.md), base case: lead time 3 days, weekly Sunday review, lost sales, 25 review dates, one run made once). The **fair benchmark is B3a**, the same point forecast with empirical residual quantiles. **Against B3a the quantile policy needs about 5% to 16% less inventory at the same fill rate for service targets 0.80 to 0.95** (comparator-anchored: the quantile policy's inventory interpolated at each B3a setting's own fill rate gives 4.5%, 7.7% and 5.6% at B3a's 0.80, 0.90 and 0.95; quantile-anchored, the pre-registered statistic: B3a's inventory interpolated at the quantile policy's own fill rate gives 9.2% and 16.2% at its 0.90 and 0.95, the others not matched) (the pooled item-bootstrap intervals exclude zero, narrowly for the comparator-anchored mean: 16.0%, [0.7%, 24.9%]; quantile-anchored 12.7%, [9.5%, 24.4%]. They include zero in the low- and high-velocity segments and in the holiday peak, and the moderate service levels were not bootstrapped one by one, so an advantage there is suggestive, not established), **and much less at 0.99** (37.7 against 69.7 units per series-day for the same fill rate; the 0.99 end is steep for the quantile policy too, 10.9 extra units per fill-rate point from 0.95 to 0.99, but steeper for B3a, 30.6). Against the pre-specified normal-sigma point policy (B2) the comparator-anchored reduction is about 29%; against the naive c x average-demand rule about 45%. The intervals reflect which items were sampled, not variation between periods (one 26-week test period). Three predictions were committed before the run: **0.99 is inefficient for the quantile policy (confirmed); a smaller edge over B3a than over the point policy (holds pooled and for high velocity, fails for mid velocity, so not confirmed); under-service in the rest period (refuted: the quantile policy served above its target at every level in both sub-periods).** At the cost ratio for a 0.80 target the naive rule had the lowest total cost (its minimum is interior after a post-hoc extension of its grid), but the other policies' minima sit at their 0.80 grid edge and cannot be extended, so the comparison of best cases is **unresolved**. **Sensitivities** (each run once, expected outcomes written first; [results](docs/results/)): lead time 7, random lead time, no future-price inputs and a hindsight diagnostic that drops zero-demand cycles. 10 of 13 expected outcomes held and 3 did not; all three concerned a pooled mean reduction that shifts with which of the four service settings could be matched. At the moderate service levels the reduction against B3a stayed between 3.5% and 9.5% in every run, at every matched setting. The point policy's normal-sigma assumption failed the constant-CV check for high-velocity series, so its deficit there partly measures that assumption.
