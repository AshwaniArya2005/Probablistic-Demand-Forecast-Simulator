...
Sharded like policy_rows.py over the units."""
import hashlib
import json
import subprocess
...
def build_unit(name, cutoff, dates, floor):
    ...
    for role, m in (("quantile", qm), ("mean", mm)):
        ...
        m.save(path)
        manifest(path, role=role, unit=name, P=P, cutoff=str(cutoff.date()), fit_end=str(versions.fit_end(cutoff).date()), floor=float(floor), n_columns=len(cols), columns=cols)
    return tab, cal_t
...
if __name__ == "__main__":
    ...
    if kind == "dev":
        ...
        if not touch_logged():
            raise SystemExit(f"refusing to build test-window forecast tables: no '{TOUCH_KEY}' row in the touch log (docs/design.md section 14)")
        units = []
        for v, c in versions.CUTOFFS.items():
            units.append((f"test_v{v}", f"sim_test_v{v}_P{P}", c, versions.use_dates(v)))
    else:
    ...
    for idx, (name, stem, cutoff, dates) in enumerate(units):
        ...
        tab, cal_t = build_unit(name, cutoff, pd.DatetimeIndex(dates), floor)
